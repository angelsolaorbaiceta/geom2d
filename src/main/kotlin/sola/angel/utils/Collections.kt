...
/**
 * Creates a new list from the given iterable by combining contiguous elements together in a pair.
 *
 * Given [A, B, C], this function would yield: [<A, B>, <B, C>]
 */
internal fun <T> makePairs(sequence: Iterable<T>): List<Pair<T, T>> {
    val seqAsList = sequence.toList()

    if (seqAsList.isEmpty()) throw Exception("Cannot create round pairs from empty collection")
    if (seqAsList.size == 1) return emptyList()

    return IntRange(1, seqAsList.size - 1)
        .map {index ->
            Pair(seqAsList[index - 1], seqAsList[index])
        }
        .toList()
}